...
public class KafkaRedisChatConsumer {
	...
	public void listenToSaveSingleFilteredRedis(ConsumerRecord<String, Object> record) {
		try {
			// 객체 만들어주기
			SingleFilteringResponse response = objectMapper.readValue(record.value().toString(),
				SingleFilteringResponse.class);
			if (response.getFilteredMessage().getFilteredType().equals(FilteredType.NOT_FILTERED))
				return;
	...
	public void listenToSaveContextFilteredRedis(ConsumerRecord<String, Object> record) {
		try {
			ContextFilteringResponse response = objectMapper.readValue(record.value().toString(),
				ContextFilteringResponse.class);
